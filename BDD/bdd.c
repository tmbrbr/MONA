...
GNUC_INLINE bdd_ptr bdd_get_free_node_sequential(bdd_manager *bddm) {
  bddm->table_elements++;
...
GNUC_INLINE unsigned bdd_find_leaf_sequential(bdd_manager *bddm, unsigned val) {
  unsigned res = bdd_get_free_node_sequential(bddm);
...
GNUC_INLINE bdd_ptr bdd_find_node_hashed_add_root(bdd_manager *bddm,
...
GNUC_INLINE bdd_handle bdd_handle_find_node_hashed_add_root(bdd_manager *bddm,
							    bdd_ptr l, bdd_ptr r, unsigned indx) {
  bdd_ptr p;
  invariant(indx <= BDD_MAX_INDEX);
  p = bdd_find_node_hashed(bddm, l, r, indx, BDD_ROOTS(bddm), NULL, NULL);
