...
static GTA *orig, *res; /* original and result GTA */
static SsId s, lSs, rSs; /* current, left and right state spaces */

static StateSet *unproc, *initial; /* for quotient */

static BehaviourMatrix *b; /* extendable behaviour matrix */
static Subsets *sets; /* subset containers */

static State *init; /* initial states */
static int *final; /* final status sequence */
static unsigned finalAllocated; /* allocated size of 'final' */
